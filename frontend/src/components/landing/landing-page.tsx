...
const steps: Array<[string, string, string, LucideIcon]> = [
  ['01', 'Create your account', 'Sign in with email or Google to start.', User],
  ['02', 'Complete KYC', 'Share identity and bank details through a guided checklist.', BadgeCheck],
  ['03', 'Apply for a loan', 'Choose an amount and tenure that fits your need.', Wallet],
  ['04', 'Eligibility check', 'We assess your request against published criteria.', BarChart3],
  ['05', 'Approval & disbursement', 'If approved, funds move to your verified bank account.', Landmark],
  ['06', 'Repayment', 'Pay EMIs from the app on the agreed schedule.', Coins],
]
...
export function LandingPage() {
  ...
  async function onContact(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch(`${base}/health/contact`, {
        method: 'POST',
  ...
  return <div className="landing-root"><main id="top" className="overflow-hidden">
    ...
    <section id="how-it-works" className="bg-[#eef5f4] px-5 py-24 sm:px-8"><div className="mx-auto max-w-7xl"><SectionHeading eyebrow="How it works" title="A clear path from need to next step." body="Our team keeps you informed and supported through every stage." /><div className="relative mt-14 grid gap-5 md:grid-cols-2 lg:grid-cols-3">{steps.map(([number, title, text, StepIcon]) => <div key={number} className="relative flex gap-4 rounded-3xl border border-white/80 bg-white/80 p-5 shadow-sm"><div className="grid size-11 shrink-0 place-items-center rounded-2xl bg-primary text-white"><StepIcon className="size-5" /></div><div><div className="flex items-center gap-2"><span className="font-mono text-[10px] font-bold text-emerald-700">{number}</span><h3 className="font-bold text-primary">{title}</h3></div><p className="mt-2 text-sm leading-6 text-muted-foreground">{text}</p></div></div>)}</div><div className="mt-10 flex flex-col items-start justify-between gap-6 rounded-3xl bg-primary p-7 text-white sm:flex-row sm:items-center sm:p-9"><div><p className="text-xs font-bold uppercase tracking-[0.15em] text-emerald-300">Apply online</p><p className="mt-3 max-w-2xl text-lg font-semibold leading-7">Create an account, complete KYC, and submit a loan application from the RupayAid portal.</p></div><div className="flex shrink-0 flex-wrap gap-2"><ButtonLink href="/login">Apply now <ArrowRight className="size-4" /></ButtonLink><ButtonLink href={whatsapp} secondary><MessageCircle className="size-4" /> WhatsApp</ButtonLink></div></div></div></section>
    ...
    <section id="referrals" className="bg-primary px-5 py-24 text-white sm:px-8"><div className="mx-auto max-w-7xl"><div className="flex flex-col justify-between gap-8 lg:flex-row lg:items-end"><SectionHeading eyebrow="Referral programme" title="Good support is worth passing on." body="Know someone who could use a little room to move? Help them take the next step." /><ButtonLink href="/login">Share your code <ArrowRight className="size-4" /></ButtonLink></div><div className="mt-14 grid gap-3 md:grid-cols-4">{['Refer someone', 'They apply', 'They complete the process', 'Eligible benefit applies'].map((item, i) => <div key={item} className="relative rounded-3xl border border-white/10 bg-white/[.07] p-6"><span className="font-mono text-xs text-emerald-300">0{i + 1}</span><div className="mt-10 text-lg font-bold">{item}</div>{i < 3 && <ArrowRight className="absolute -right-4 top-1/2 z-10 hidden size-5 text-emerald-300 md:block" />}</div>)}</div></div></section>
